...
# Canonical consumption channels (allowlist) — the three real channels of the
# PodID contract (dbo.PredictiveInputData). The single source of truth for the
# forecasting path: only these are ever fitted, so identifier/metadata columns
# (UserForecastMethodID, TariffSubType, …) can never leak in as channels.
CONSUMPTION_COLUMNS = [
    "PeakConsumption", "StandardConsumption", "OffPeakConsumption",
]

# Wider legacy set: bundled DB frames and the retired entity-keyed fixtures also
# carry Block1-4/NonTOU consumption (SPU entities store their energy in NonTOU).
# Display/harness code (dashboards, in-sample engine) keys off this list so those
# artifacts still render fully; the forecasting path uses CONSUMPTION_COLUMNS only.
LEGACY_CONSUMPTION_COLUMNS = CONSUMPTION_COLUMNS + [
...
def consumption_columns(series: pd.DataFrame) -> list:
    """Consumption value columns present in ``series``, in canonical order.

    Allowlist intersection: only :data:`CONSUMPTION_COLUMNS` members qualify,
    so an identifier or metadata column can never be picked up as a forecast
    channel. Shared by the validator and the forecasting algorithms so the two
    never drift out of sync.
    """
    return [c for c in CONSUMPTION_COLUMNS if c in series.columns]
...
def validate_series(unit: PredictionUnit, method: str,
                    cons_cols: list | None = None) -> tuple[bool, str]:
    ...
    ``cons_cols`` overrides the allowlist-based channel discovery for callers
    whose series carries a non-canonical consumption column (the bundle
    aggregate); leave it None to derive from :func:`consumption_columns`.
    """
    series = unit.series
