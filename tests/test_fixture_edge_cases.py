...
class TestLoopRoutesEdgeEntities:
    ...
    def test_entity_types_correctly_assigned(self, loop_run):
        _, _, constructed = loop_run
        units = {u.entity_id: u for u in constructed}
        assert units["POD_SHORT_SERIES"].entity_type  == "POD"
        assert units["COMBO_ZERO_CONSUMPTION"].entity_type == "Combo"
        assert units["POD_OUTLIER_SPIKE"].entity_type == "POD"
